...
impl WorkspaceAdapter for LocalWorkspaceAdapter {
    fn execute_node(
        ...
        let (artifact_refs, fact_candidates, verification_evidence, recovery, side_effect_summary) = match node_type {
            ...
            "commit_external_send" => {
                ...
                let compensation_path = artifacts_dir.join(format!("{node_id}_compensation_plan.json"));
                let message = json!({
                ...
                outbox.as_array_mut().expect("outbox should be an array").push(message.clone());
                ...
                (
                    vec![artifact_ref(&send_artifact), artifact_ref(&compensation_path)],
                    Vec::new(),
    ...
    fn execute_recovery(
        ...
        match recovery.action.as_str() {
            "compensate" => {
                ...
                outbox.as_array_mut().expect("outbox should be an array").push(correction.clone());
                write_json(&outbox_path, &outbox)?;
                crm_record["history"]
                    .as_array_mut()
                    .expect("crm history should be an array")
                ...
                write_json(&crm_path, &crm_record)?;
                let artifact_path = artifacts_dir.join(format!("{}_compensation_executed.json", recovery.node_id));
            ...
            "rollback" => {
                let restore_from = policy["restore_from"]
                    .as_str()
                    .ok_or_else(|| CoreError::Serialization("rollback policy missing restore_from".to_owned()))?;
                let restore_path = artifacts_dir.join(restore_from);
                let baseline = read_json(&restore_path)?;
                write_json(&crm_path, &baseline)?;
                let artifact_path = artifacts_dir.join(format!("{}_rollback_executed.json", recovery.node_id));
                write_json(
                    &artifact_path,
                    &json!({
...
fn write_json(path: &Path, payload: &Value) -> CoreResult<()> {
    ...
    let serialized =
        serde_json::to_string_pretty(payload).map_err(|error| CoreError::Serialization(error.to_string()))?;
