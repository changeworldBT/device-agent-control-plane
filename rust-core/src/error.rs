...
impl Display for CoreError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UnsupportedEventType(event_type) => write!(formatter, "unsupported event type: {event_type}"),
            Self::IllegalTaskTransition { from, to } => {
                write!(formatter, "illegal task transition: {from} -> {to}")
            }
            Self::VerificationRequired { from, to } => {
                write!(formatter, "verification required for transition: {from} -> {to}")
            }
            Self::DispatchDenied { task_id, node_id } => {
                write!(formatter, "dispatch denied: no active grant for task={task_id} node={node_id}")
            }
            Self::ApprovalDenied { task_id, node_id } => {
                write!(formatter, "grant denied: approval required for task={task_id} node={node_id}")
            }
